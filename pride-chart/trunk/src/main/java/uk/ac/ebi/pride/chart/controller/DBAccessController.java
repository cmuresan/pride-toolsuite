...
public class DBAccessController {
    ...
    public List<PrecursorData> precursorMassData(String accessionNumber) {
        ...
        try {
            String query = "select exp.accession, spec.spectrum_id, param.value " +
                    "from pride_experiment exp, mzdata_mz_data mz, mzdata_spectrum spec, " +
                    "     mzdata_precursor prec, mzdata_ion_selection_param param " +
                    "where mz.accession_number = ? and " +
                    "      exp.accession = mz.accession_number and " +
                    "      mz.mz_data_id = spec.mz_data_id and " +
                    "      spec.spectrum_id = prec.spectrum_id and " +
                    "      prec.precursor_id = param.parent_element_fk and " +
                    "      spec.ms_level = 2 and " +
                    "      (param.accession = 'MS:1000744' or param.accession = 'PSI:1000040')";
            ...
            while (rs.next()) {
                is_empty = false;
                try {
                    precursorDataList.add(new PrecursorData(
                            rs.getString("accession"),
                            rs.getString("spectrum_id"),
                            rs.getString("value")));
                }
                catch (PrecursorDataException e) {/*Nothing here*/
                    logger.error(e.getMessage(), e);
                }
            }
            rs.close();
    ...
    public List<PrecursorData> precursorChargeData(String accessionNumber) {
        ...
        try {
            PreparedStatement st = DBConnection.prepareStatement(
                    "select exp.accession, spec.spectrum_id, param.value " +
                            "from pride_experiment exp, mzdata_mz_data mz, mzdata_spectrum spec, " +
                            "     mzdata_precursor prec, mzdata_ion_selection_param param " +
                            "where mz.accession_number = ? and " +
                            "      exp.accession = mz.accession_number and " +
                            ...
                            "      prec.precursor_id = param.parent_element_fk and " +
                            "      spec.ms_level = 2 and " +
                            "      (param.accession = 'MS:1000041' or param.accession = 'PSI:1000041') ");
    ...
    // Query if the spectrum is identified for each spectrum_id
    ...
    public Map<String, Boolean> precursorSpectrumIdentified(String accessionNumber) {
        ...
        try {
            PreparedStatement st = DBConnection.prepareStatement(
                    "select spec.spectrum_id, spec.spectrum_identifier " +
                            "from pride_experiment exp, mzdata_mz_data mz, mzdata_spectrum spec " +
                            "where exp.accession = ? and " +
                            "      exp.accession = mz.accession_number and " +
                            "      mz.mz_data_id = spec.mz_data_id");
    ...
    private Set<String> getIdentifiedPeptidesReferences(String accessionNumber){
        ...
        try {
            PreparedStatement st = DBConnection.prepareStatement(
                    "select pep.spectrum_ref " +
                            "from pride_experiment exp, pride_identification ide, pride_peptide pep " +
                            "where exp.accession = ? and " +
                            "      exp.experiment_id = ide.experiment_id and " +
                            "      ide.identification_id = pep.identification_id");
    ...
    //Peak List MZ Data Query

    public PeakListBasicInfo peakListMzInfo(String accessionNumber) {
        PeakListBasicInfo peakListBasicInfo = null;
        try {
            PreparedStatement st = DBConnection.prepareStatement(
                    "select s.spectrum_id, c.data_endian, a.binary_array_id, c.data_precision " +
                            "from mzdata_mz_data m, mzdata_spectrum s, mzdata_binary_array c, mzdata_base_64_data a " +
                            "where m.mz_data_id=s.mz_data_id and " +
                            "   c.binary_array_id=s.mz_array_binary_id and " +
                            "   c.binary_array_id=a.binary_array_id and " +
                            "   m.accession_number = ? and " +
                            "   s.ms_level= 2 " +
                            "group by a.binary_array_id " +
                            "order by s.spectrum_id "); //Needed for retrieving data of mz and intensity in the same order
            st.setString(1, accessionNumber);
        ...
        try {
            PreparedStatement st = DBConnection.prepareStatement(
                    "select s.spectrum_id, c.data_endian, a.binary_array_id, c. data_precision " +
                            "from mzdata_mz_data m, mzdata_spectrum s, mzdata_binary_array c, mzdata_base_64_data a " +
                            "where m.mz_data_id=s.mz_data_id and " +
                            "   c.binary_array_id=s.inten_array_binary_id and " +
                            "   c.binary_array_id=a.binary_array_id and " +
                            "   m.accession_number = ? and " +
                            "   s.ms_level= 2 " +
                            "group by a.binary_array_id " +
    ...
    public Map<Integer, String> getBinaryData(List<Integer> array_binary_ids) {
        ...
        try {
            PreparedStatement st = DBConnection.prepareStatement(
                    "SELECT binary_array_id, base_64_data " +
                            "FROM mzdata_base_64_data " +
                            "WHERE binary_array_id IN (" + arrayBinaryIDs + ")");
            ResultSet rs = st.executeQuery();
            boolean is_empty = true;
            while (rs.next()) {
                is_empty = false;
    ...
    public List<ProteinPeptide> getProteinsIdentification(String accessionNumber) {
        ...
        try {
            PreparedStatement st = DBConnection.prepareStatement(
                    "select ide.identification_id protein_ID, pep.peptide_id, pep.sequence, pep.spectrum_ref " +
                            "from pride_identification ide, pride_peptide pep , pride_experiment exp " +
                            "where exp.accession = ? AND " +
                            "   ide.experiment_id = exp.experiment_id AND " +
                            "   ide.identification_id = pep.identification_id ");
            st.setString(1, accessionNumber);

            ResultSet rs = st.executeQuery();
            //Will be used for getting the SPECTRUM IDs
            List<Integer> spectrumRefs = new ArrayList<Integer>();
            //Will be used for getting the PTM masses for each peptide_id
            ...
            while (rs.next()) {
                ...
                DBProteinPeptide dbpp = new DBProteinPeptide(
                        rs.getInt("protein_ID"),
                        peptideID,
                        rs.getString("sequence"),
                        spectrumRef);
                if(peptideScores.containsKey(peptideID))
                    dbpp.setPeptideScore(peptideScores.get(peptideID));
            ...
            if(spectrumRefs.size()>0 && peptideIDs.size()>0){
                //Contains the equivalent SPECTRUM ID foreach SPECTRUM REF in the experiment
                Map<Integer, Integer> spectrumsIDByRefs = getSpectrumsIDByRefs(spectrumRefs, accessionNumber);
                //Contains the sum of the PTM foreach peptide ID
                ...
                for(ProteinPeptide pp : proteinsPeptides){
                    ...
                    //Converting spectrum references stored for the spectrum identifiers
                    int spectrumRef = dbPP.getSpectrumRef();
                    if(spectrumsIDByRefs.keySet().contains(spectrumRef))
                        dbPP.setSpectrumID(spectrumsIDByRefs.get(spectrumRef));
                    //else
                        //ToDo: decide what to do if this happens, because it means the spectrumID is wrong

                    //Set the sum of all the PTM mass
                    int peptideID = dbPP.getPeptideID();
                    if(monoMassMod.keySet().contains(peptideID))
                        dbPP.setPtmMass(monoMassMod.get(peptideID));
                }
            }
        } catch (SQLException e) {
            logger.error(e.getMessage(), e);
    ...
    private Map<Integer, Integer> getSpectrumsIDByRefs(List<Integer> spectrum_refs, String accessionNumber) {
        ...
        try {
            PreparedStatement st = DBConnection.prepareStatement(
                    "select ms.spectrum_identifier, ms.spectrum_id " +
                            "from mzdata_spectrum ms, mzdata_mz_data mz " +
                            "where mz.accession_number = ? and " +
                            "   mz.mz_data_id = ms.mz_data_id and " +
                            "   ms.spectrum_identifier in ("+spectrumRefs+")");
            st.setString(1, accessionNumber);
            ResultSet rs = st.executeQuery();
            boolean is_empty = true;
            while (rs.next()) {
            ...
            //Failed to query spectrum by spectrum references
            logger.error(e.getMessage(), e);
        }
    ...
    private Map<Integer, Double> getPeptidesMonoMassModification(List<Integer> peptides_id) {
        ...
        try {
            PreparedStatement st = DBConnection.prepareStatement(
                    "select m.peptide_id, d.mass_delta_value " +
                            "from pride_modification m, pride_mass_delta d " +
                            "where m.peptide_id in (" + peptidesIDs + ") and " +
                            "   m.modification_id = d.modification_id and " +
                            "   d.classname = ?");
    ...
    private Map<Integer, PeptideScore> getPeptideScore(String accessionNumber){
        ...
        try {
            PreparedStatement st = DBConnection.prepareStatement(
                    "SELECT peptide_id, ppp.accession, value " +
                            "FROM pride_peptide_param as ppp join pride_peptide on(parent_element_fk=peptide_id)" +
                            "     join pride_identification using(identification_id) " +
                            "     join pride_experiment using(experiment_id) " +
                            "WHERE pride_experiment.accession=?"
                    );
            st.setString(1, accessionNumber);
            ResultSet rs = st.executeQuery();
            boolean is_empty = true;
    ...
    // Returns -1 if the accession number doesn't match with any experiment

    public int getExperimentID(String accessionNumber){
        int experimentID = -1;
        try {
            PreparedStatement st = DBConnection.prepareStatement(
                    "select experiment_id from pride_experiment where accession = ?");
            st.setString(1, accessionNumber);
            ResultSet rs = st.executeQuery();
    ...
    public boolean storeChartData(int experimentID, int type, String data){
        ...
        try {
            PreparedStatement st = DBConnection.prepareStatement(
                    "insert into pride_chart_data(experiment_id, chart_type, intermediate_data)" +
                            "values (?, ?, ?)" +
                            "on duplicate key update intermediate_data = ?");
    ...
    public List<PrideChart> getChartData(int experimentID){
        ...
        try {
            PreparedStatement st = DBConnection.prepareStatement(
                    //ToDo: Change the query when the old data format will be removed from the database
                    "select chart_type, intermediate_data from pride_chart_data where experiment_id = ? and intermediate_data is not null order by chart_type");
            st.setInt(1, experimentID);
