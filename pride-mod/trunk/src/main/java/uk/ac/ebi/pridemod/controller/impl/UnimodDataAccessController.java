...
import uk.ac.ebi.pridemod.io.unimod.model.Specificity;
import uk.ac.ebi.pridemod.io.unimod.model.Unimod;
import uk.ac.ebi.pridemod.io.unimod.model.UnimodModification;
...
public class UnimodDataAccessController extends AbstractDataAccessController{
    ...
    private void initPTMMap(Unimod unimodObject) {
        ...
        for(UnimodModification unimodMod: unimodObject.getModifications().getMod()){
            ...
            List<uk.ac.ebi.pridemod.model.Specificity> specificityList = null;

            if(unimodMod.getSpecificity() != null && unimodMod.getSpecificity().size() > 0){
                specificityList = new ArrayList<uk.ac.ebi.pridemod.model.Specificity>(unimodMod.getSpecificity().size());
                for(Specificity oldSpecificty: unimodMod.getSpecificity()){
                    uk.ac.ebi.pridemod.model.Specificity specificity = new uk.ac.ebi.pridemod.model.Specificity(oldSpecificty.getSite(), oldSpecificty.getPosition());
