...
public class PrideXmlControllerImplTest {
    ...
    public void testGetSpectrumById() throws Exception {
        ...
        assertEquals("Precursor spectrum ref", spectrum.getPrecursors().get(0).getSpectrum().getId(), "0");
    ...
    @Test
    public void testGetTwoDimIdentIds() throws Exception {
        assertTrue("There should be two two dimensional identification", prideController.getTwoDimIdentIds().size() == 2);
    }

    @Test
    public void testGetTwoDimIdentById() throws Exception {
        TwoDimIdentification ident = prideController.getTwoDimIdentById("2");
        assertEquals("Accession version", ident.getAccessionVersion(), "2");
        // peptide
        assertEquals("Peptide sequence", ident.getPeptides().get(0).getSequence(), "GPAGEPMGPEAGSK");
        // modification
        assertEquals("Modification", ident.getPeptides().get(0).getModifications().get(0).getModDatabase(), "UniMod");
        // gel location
        assertEquals("Gel location", ident.getGel().getXCoordinate(), 3.141593E0);
        // mol weight
        assertEquals("Mol weight", ident.getGel().getMolecularWeight(), 3.141593E0);
    }

    @Test
    public void testGetGelFreeIdentIds() throws Exception {
        assertTrue("There should be only one gel free identification", prideController.getGelFreeIdentIds().size() == 1);
    }

    @Test
    public void testGetGelFreeIdentById() throws Exception {
        GelFreeIdentification ident = prideController.getGelFreeIdentById("0");
        assertEquals("Accession number should be P02769", ident.getAccession(), "P02769");

        // peptide
        assertEquals("Pepeptide sequence", ident.getPeptides().get(0).getSequence(), "LFTFHADICTLPDTEK");
        assertEquals("Spectrum reference", ident.getPeptides().get(0).getSpectrum().getId(), "83");

        // modification
        assertEquals("Modification location", ident.getPeptides().get(0).getModifications().get(0).getLocation(), 9);
        assertEquals("Modification param", ident.getPeptides().get(0).getModifications().get(0).getCvParams().get(0).getAccession(), "MOD:01214");

        // fragment ion
        assertEquals("Fragment ion", ident.getPeptides().get(0).getFragmentIons().get(0).getCvParams().get(0).getAccession(), "PRIDE:0000204");
    }
}
