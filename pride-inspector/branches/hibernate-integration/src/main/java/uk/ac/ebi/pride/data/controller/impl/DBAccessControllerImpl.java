...
public class DBAccessControllerImpl extends AbstractDataAccessController {
    ...
    private static String CONTACT_INFO = "contact information";
    private static String SAMPLE_ID = "sample1";
    private static String COMMENTS = "comments";
    private static String COMPLETION_TIME = "completion time";
    private static int PROCESSING_METHOD_ORDER = 1;
    private static String DATA_PROCESSING_ID = "dataprocessing1";
    private static String PROTOCOL_ID = "protocol1";
    ...
    public DBAccessControllerImpl() throws DataAccessException {
        ...
        //    URL url = DBAccessControllerImpl.class.getClassLoader().getResource("database.properties");
        try {
        ...
        List<Comparable> experimentIds = getExperimentIds();
        setForegroundExperimentId(experimentIds.get(0));
        //only for testing purposes
        //setForegroundExperimentId("8500");
        //setForegroundExperimentId("10470");   //experiment with identifications
    }

    public List<CvParam> getCvParams(String table_name, int parent_element_id) {
        List<CvParam> cvParams = new ArrayList<CvParam>();
        try {
            PreparedStatement st = DBConnection.prepareStatement("SELECT accession, name, value FROM " + table_name + " WHERE parent_element_fk = ? AND cv_label is not null");
            st.setInt(1, parent_element_id);
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
                cvParams.add(new CvParam(rs.getString("accession"), rs.getString("name"), "", rs.getString("value"), "", "", ""));
            }
            rs.close();
        }
    ...
    public List<SourceFile> getSourceFiles() {
        ...
        try {
            ...
            st.setString(1, foregroundExperimentId.toString());
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
    ...
    public Protocol getProtocol() {
        ...
        protocol_steps = getProtocolStepsById(Integer.parseInt(foregroundExperimentId.toString()));
    ...
    public List<Reference> getReferences() {
        ...
        try {
            ...
            st.setInt(1, Integer.parseInt(foregroundExperimentId.toString()));
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
    ...
    public Experiment getExperimentById(Comparable experimentId) throws DataAccessException {
        return null;
    }
